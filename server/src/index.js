...
app.post('/api/invitations/:token/accept', asyncRoute(async (req, res) => {
  ...
  const result = await prisma.$transaction(async (tx) => {
    ...
    await tx.organisationMembership.upsert({ where: { organisationId_userId: { organisationId: invitation.organisationId, userId: user.id } }, create: { organisationId: invitation.organisationId, userId: user.id, role: invitation.type === 'PLAYER' ? 'STAFF' : 'STAFF' }, update: { status: 'ACTIVE' } });
